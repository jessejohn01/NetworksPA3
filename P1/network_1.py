...
class NetworkPacket:
    ...
    def from_byte_S(self, byte_S):
        ...
        packetID = int(byte_S[NetworkPacket.dst_addr_S_length: (
                    NetworkPacket.dst_addr_S_length + NetworkPacket.packetIDLength)])  # From the end of the address to the end of ID length.
        endFlag = int(byte_S[(NetworkPacket.dst_addr_S_length + NetworkPacket.packetIDLength):((
                    NetworkPacket.dst_addr_S_length + NetworkPacket.packetIDLength) + NetworkPacket.lastPacketFlagLength)])
        data_S = byte_S[((
                    NetworkPacket.dst_addr_S_length + NetworkPacket.packetIDLength) + NetworkPacket.lastPacketFlagLength):]  # The rest is data.
...
class Host:
    ...
    def udt_send(self, dst_addr, data_S):
        ...
        for x in range(len(packetList)):
            ...
            self, packetList[x], self.out_intf_L[0].mtu))
        self.packetIDGen += 1
            ...
            try:
                ...
                if pkt_S is not None:
                    ...
                    for x in range(len(packetList)):
                        ...
                        self, packetList[x], i, i, self.out_intf_L[i].mtu))
            except queue.Full:
